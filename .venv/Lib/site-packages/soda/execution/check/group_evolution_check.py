...
class GroupEvolutionCheck(Check):
    ...
    def evaluate(self, metrics: dict[str, Metric], historic_values: dict[str, object]):
        ...
        if previous_groups:
            ...
            self.group_comparator = None
            if group_evolution_check_cfg.has_change_validations():
                warning_message = f"Skipping group evolution changes for '{self.partition.table.table_name}' since there are no historic groups available!"
                self.logs.warning(warning_message)

                if not group_evolution_check_cfg.has_required_forbidden_groups():
        ...
        if validations.forbidden_group_names:
            for forbidden_group_name in validations.forbidden_group_names:
                ...
                for group_name in measured_groups:
                    if forbidden_pattern.match(str(group_name)):
