...
PROJECT_ROOT = "/home/user/projects/newproj/soda/"
...
def run_soda_scan(project_root, scan_name, checks_subpath = None):
    ...
    # this commented code is the one that threw errors when executing the DAG
    # checks_transform returns 1 when it detects 209 rows don't have a category or subcategory
    # if result != 0:
...
with DAG(
    ...
    dbt_transform = BashOperator(
        ...
        bash_command=f"dbt run --project-dir {PROJECT_ROOT}/dbt/partner_cert --select transform",
    )
    ...
    dbt_report = BashOperator(
        task_id="dbt_report",
        bash_command=f"dbt run --project-dir {PROJECT_ROOT}/dbt/partner_cert --select report",
    )
